...
for data in lines:
	...
	filteredSentence = [w for w in tokenizedWords if not w in sw and not w in set(string.punctuation)]
...
X = np.asarray(X)
NUM_CLUSTERS = 10
kclusterer = KMeansClusterer(NUM_CLUSTERS, distance=nltk.cluster.util.cosine_distance, repeats=5, avoid_empty_clusters=True)
